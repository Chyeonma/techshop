...
public class PaymentsController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateVnPayPayment(CreatePaymentRequestDto dto)
    {
        ...
        if (!response.Success) return response.Error == "ORDER_NOT_FOUND" ? NotFound(response) : BadRequest(response);
        return Ok(response);
    }
    ...
    {
        var response = await _paymentsService.CreateGatewayPaymentAsync(dto, "QR");
        if (!response.Success) return response.Error == "ORDER_NOT_FOUND" ? NotFound(response) : BadRequest(response);
        return Ok(response);
    }
    ...
    {
        var response = await _paymentsService.CreateGatewayPaymentAsync(dto, "ZaloPay");
        if (!response.Success) return response.Error == "ORDER_NOT_FOUND" ? NotFound(response) : BadRequest(response);
        return Ok(response);
    }
    ...
    public async Task<IActionResult> VnPayCallback(PaymentCallbackDto dto)
    {
        ...
        if (!response.Success) return response.Error == "PAYMENT_NOT_FOUND" ? NotFound(response) : BadRequest(response);
        return Ok(response);
    }
    ...
    {
        var response = await _paymentsService.HandleCallbackAsync(dto, "QR");
        if (!response.Success) return response.Error == "PAYMENT_NOT_FOUND" ? NotFound(response) : BadRequest(response);
        return Ok(response);
    }
    ...
    {
        var response = await _paymentsService.HandleCallbackAsync(dto, "ZaloPay");
        if (!response.Success) return response.Error == "PAYMENT_NOT_FOUND" ? NotFound(response) : BadRequest(response);
        return Ok(response);
    }
}
